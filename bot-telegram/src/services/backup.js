...
const BACKUP_DIR = path.join(BOT_ROOT, 'backups');
...
class BackupManager {
  ...
  async createBackup(options = {}) {
    ...
    try {
      // Backup products
      if (includeProducts) {
        const productsFile = path.join(BOT_ROOT, 'data', 'products.json');
      ...
      // Backup payments
      if (includePayments) {
        const paymentsFile = path.join(BOT_ROOT, 'data', 'payments.json');
      ...
      // Backup settings
      if (includeSettings) {
        const settingsFile = path.join(BOT_ROOT, 'data', 'settings.json');
      ...
      if (includeState) {
        const stateFile = path.join(BOT_ROOT, 'data', 'bot-state.json');
