 ...
 */
export async function setupProductRefreshJob(intervalMinutes = 5) {
  ...
  scheduler.addJob(
    ...
    async () => {
      ...
      await loadProducts(true, { resetStability: true }); // Force refresh + reset stability
...
export async function setupCleanupJob(intervalHours = 24) {
  const { USER_SESSIONS } = await import('../bot/state.js');
  ...
  scheduler.addJob(
    ...
    async () => {
      ...
      let cleanedCount = 0;
      for (const [userId, userData] of USER_SESSIONS.entries()) {
