...
export default function ProductsPage() {
  ...
  const handleToggleAktif = async (productId: string, currentStatus: boolean) => {
    try {
      ...
      try {
        ...
        console.log('✅ Bot refreshed after status toggle')
      } catch (refreshErr) {
        console.warn('⚠️ Failed to trigger bot refresh:', refreshErr)
      }
    } catch (error: any) {
      console.error('Error toggling aktif status:', error)
                    ...
                    <tr key={product.id} className={`hover:bg-gray-50 transition ${
                      ...
                      <td className="px-6 py-3 text-center">
                        <button
                          ...
                          className={`inline-block px-3 py-1 rounded-full text-sm font-medium cursor-pointer transition-all hover:opacity-80 ${
                            product.aktif ? 'bg-green-100 text-green-800 hover:bg-green-200' : 'bg-gray-100 text-gray-800 hover:bg-gray-200'
                          }`}
                ...
                <div key={product.id} className={`p-4 hover:bg-gray-50 ${
                  ...
                  <div className="mt-3">
                    ...
                    <div className="mt-1">
                      <button
                        onClick={() => handleToggleAktif(product.id, product.aktif)}
                        className={`inline-block px-3 py-1 rounded-full text-xs font-medium cursor-pointer transition-all hover:opacity-80 ${
                          product.aktif ? 'bg-green-100 text-green-800 hover:bg-green-200' : 'bg-gray-100 text-gray-800 hover:bg-gray-200'
                        }`}
                        title="Click to toggle status"
                      >
          ...
          <div className="bg-white rounded-lg shadow-xl p-8 w-full max-w-md max-h-[90vh] overflow-y-auto">
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <div className="border-t pt-4">
                <label className="block text-sm font-medium text-gray-700 mb-3">Status Produk</label>
                <div className="flex items-center gap-3">
                  <button
                    type="button"
                    onClick={() => setFormData({ ...formData, aktif: !formData.aktif })}
                    className={`relative inline-flex h-8 w-14 items-center rounded-full transition-colors ${
                      formData.aktif ? 'bg-green-500' : 'bg-gray-300'
                    }`}
                  >
                    <span
                      className={`inline-block h-6 w-6 transform rounded-full bg-white transition-transform ${
                        formData.aktif ? 'translate-x-7' : 'translate-x-1'
                      }`}
                    />
                  </button>
                  <span className={`text-sm font-medium ${
                    formData.aktif ? 'text-green-600' : 'text-gray-600'
                  }`}>
                    {formData.aktif ? 'Aktif (tersedia di katalog)' : 'Nonaktif (tidak ditampilkan)'}
                  </span>
                </div>
                <p className="text-xs text-gray-500 mt-2">Jika nonaktif, produk tidak akan ditampilkan di toko online maupun bot telegram.</p>
              </div>
